...
class QueryHandler{
	...
	public function __construct(){
		...
		$this->server->getLogger()->info($this->server->getLanguage()->translateString("softmine.server.query.start"));
		$addr = ($ip = $this->server->getIp()) != "" ? $ip : "0.0.0.0";
		$port = $this->server->getPort();
		$this->server->getLogger()->info($this->server->getLanguage()->translateString("softmine.server.query.info", [$port]));
		/*
		...
		*/
		...
		$this->regenerateInfo();
		$this->server->getLogger()->info($this->server->getLanguage()->translateString("softmine.server.query.running", [$addr, $port]));
	}
