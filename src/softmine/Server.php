...
class Server{
	...
	public function getMotd(){
		return $this->getConfigString("motd", "SoftMine: MCPE Server");
	}
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath){
		...
		try{
			...
			if(!file_exists($this->dataPath . "mobAI.yml")){
				$content = file_get_contents($this->filePath . "src/softmine/resources/mobAI.yml");
				if($version->isDev()){
					$content = str_replace("preferred-channel: stable", "preferred-channel: beta", $content);
				}
				@file_put_contents($this->dataPath . "mobAI.yml", $content);
			}
			$this->config = new Config($this->dataPath . "softmine.yml", Config::YAML, []);
			$this->config = new Config($this->dataPath . "mobAI.yml", Config::YAML, []);

			$this->logger->info("Loading server properties...");
			$this->properties = new Config($this->dataPath . "server.properties", Config::PROPERTIES, [
				"motd" => "SoftMine: MCPE Server",
				"server-port" => 19132,
				"white-list" => false,
